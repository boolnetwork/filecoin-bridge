...
pub fn sign(url:&str, message:&str, pubkey:Vec<u8>) -> Result<(SK,SK,FE,FE,u8)> {

    save_url(String::from(url));
    init();
    let store = find_store(pubkey);
    ...
    let data = fs::read_to_string(store)
...
//    match message_check(&client, Message{ key:ripemd160(message).to_string()}){
//        Ok(()) => { println!("message_check Ok !"); },
//        Err(()) => {
//            println!("message_check Fail !");
//            return Err("error");
//        },
//    }
    ...
    let (party_num_int, uuid) = match signup(&client,Message{ key:ripemd160(message).to_string()}){
        ...
        Err(_) =>  { return Err(TssError::SignUp().into()) },
    };
...
pub fn signup(client: &Client,message:Message) -> Result<PartySignup, ()> {
    let key = "signup-sign".to_string();

    let res_body = postb(&client, "signupsign", message).unwrap();
    serde_json::from_str(&res_body).unwrap()
}

pub fn message_check(client: &Client,message:Message) -> Result<(), ()> {
    let res_body = postb(&client, "message", message).unwrap();
    serde_json::from_str(&res_body).unwrap()
}
...
use bridge::{SuperviseClient, TxMessage, TxSender, PacketNonce, TxType, TokenType};
...
use lotus_api::{Http, types::tipset::TipSet,types::message::BlockMessages, types::address::Address,
                api::ChainApi};
use std::time;
    ...
    fn sign_tss_message_start(self) -> impl Future<Output=()> + 'static {
        ...
        let stream = {
            self.reciver.for_each( move |(who,value)|{
                spv.submit_fc_transfer_tss( TxMessage::new(
                    TxType::FCDeposit(who,TokenType::FC,value)
                ));
        ...
        Block::Hash: Into<sp_core::H256>
{
    ...
    let tx_sender = TxSender::new(
        ...
        pool,
        key,
        Arc::new(Mutex::new(PacketNonce {nonce:0,last_block:at})),
    );
...
pub fn main_loop(sender: mpsc::UnboundedSender<(Vec<u8>,Value)>,mut reciver: FcPubkeySender,){
    thread::spawn(move || {
        ...
        loop {
            ...
            let mut message_set =
                HashMap::<Vec<u8>,(Vec<u8>,u128)>::new();
            let cids = ret.cids.clone();
            for cid in cids {
                println!("cids = {:?}",cid);
                let block_messages:BlockMessages = rt.block_on(http.chain_get_block_messages(&cid)).unwrap();
...
mod tests {
    ...
    fn test() {
        ...
        let ret: TipSet = rt.block_on(http.chain_head()).unwrap();
        let cids = ret.cids.clone();
        let ret = rt.block_on(http.chain_get_block_messages(&cids[0])).unwrap();
